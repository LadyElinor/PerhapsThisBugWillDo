...
ROOT = Path("results/reachability_sweep")
ROOT.mkdir(parents=True, exist_ok=True)
DATE = "2026-02-15-sweep"
SEEDS = 60
TPROBE = 500
EPS = 1e-12
...
def run_cell(cell_dir: Path, barrier_mult: float, access_mult: float, low_t: int):
    anneal_t, tempered_t = (0.6, 0.8) if low_t else (0.9, 1.2)
    cmd = [
        "python",
        "scripts/pilot_runner.py",
        "--chosen",
        "results/reachability/preflight/chosen_settings.json",
        "--out-root",
        str(cell_dir).replace("\\", "/"),
        "--date",
        DATE,
        "--seeds",
        str(SEEDS),
        "--physics-mode-signature",
        "mode_energy",
        "--physics-energy-bins",
        "4",
        "--physics-probe-grid",
        str(TPROBE),
        "--physics-barrier-mult",
        str(barrier_mult),
        "--physics-access-mult",
        str(access_mult),
        "--anneal-temp",
    ...
    # features at t=500, anneal+tempered only
    rows = []
    ...
    f = f[(f["tau"].isna()) | (f["tau"] > TPROBE)].copy()  # at risk
    ...
    # combined logit on finite subset
    auc_c = np.nan
...
def main():
    cells = []
    cell_id = 0
    for barrier_mult, access_mult, low_t in itertools.product([1.0, 2.0], [1.0, 0.7], [0, 1]):
        cell_id += 1
        cid = f"cell_{cell_id:02d}"
        out = ROOT / cid
        out.mkdir(parents=True, exist_ok=True)
        print(f"Running {cid}: barrier_mult={barrier_mult}, access_mult={access_mult}, low_t={low_t}")
        m = run_cell(out, barrier_mult, access_mult, low_t)
        cells.append(
            {
                "cell_id": cid,
    ...
    df = pd.DataFrame(cells).sort_values(["barrier_mult", "access_mult", "temp_level"])
    out_csv = ROOT / "sweep_summary_t500.csv"
    df.to_csv(out_csv, index=False)

    md = ["# 8-cell Sweep Summary (t=500)", "", "```", df.to_string(index=False), "```", ""]
    md.append("Legend: ONSET-DOMINATED (<0.02), TRANSITIONAL (0.02-0.05), BARRIER-EMERGENT (>=0.05)")
    out_md = ROOT / "SWEEP_READOUT.md"
